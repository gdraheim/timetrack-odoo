...
class netcTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        assert TEMP_NETRC
        dotnetrc.NETRC_FILENAMES = [TEMP_NETRC]
        logg.info("netrc -> %s", dotnetrc.NETRC_FILENAMES)
    ...
    def tearDown(self) -> None:
        ...
        TEMP_NETRC = None
        dotnetrc.NETRC_OVERRIDE = {}
        dotnetrc.NETRC_USERNAME = ""
        dotnetrc.NETRC_PASSWORD = ""
        dotnetrc.NETRC_CLEARTEXT = True
    def test_034(self) -> None:
        obf = dotnetrc._encode64("your text")
        self.assertEqual(obf, 'eW91ciB0ZXh0')
        txt = dotnetrc._decode64(obf)
        self.assertEqual(txt, "your text")
    def test_035(self) -> None:
        obf = dotnetrc._encode46("your text")
        self.assertEqual(obf, '0hXZ0Bic19We')
        txt = dotnetrc._decode46(obf)
        self.assertEqual(txt, "your text")
    def test_036(self) -> None:
        obf = dotnetrc._encode36("your text")
        self.assertEqual(obf, '0uKM0Ovp19Jr')
        txt = dotnetrc._decode36(obf)
        self.assertEqual(txt, "your text")
    def test_037(self) -> None:
        obf = dotnetrc._encode63("your text")
        self.assertEqual(obf, 'rJ91pvO0MKu0')
        txt = dotnetrc._decode63(obf)
        self.assertEqual(txt, "your text")
    def test_040(self) -> None:
        us, pw = dotnetrc.get_username_password(url_div)
        self.assertEqual(us, "testuser")
        self.assertEqual(pw, "")
    def test_050(self) -> None:
        dotnetrc.set_username_password("abc", "xyz")
        us, pw = dotnetrc.get_username_password(url_div)
        self.assertEqual(us, "abc")
        self.assertEqual(pw, "xyz")
    def test_051(self) -> None:
        dotnetrc.set_username_password("abc", "xyz")
        us, pw = dotnetrc.get_username_password(url_k8s)
        self.assertEqual(us, "abc")
        self.assertEqual(pw, "xyz")
    def test_052(self) -> None:
        dotnetrc.set_username_password("abc", "xyz")
        dotnetrc.set_username_password("abc", "uvw")
        us, pw = dotnetrc.get_username_password(url_div)
        self.assertEqual(us, "abc")
    ...
    def test_060(self) -> None:
        dotnetrc.set_username_password("abc", "xyz")
        dotnetrc.set_username_password("abc", "uvw")
        dotnetrc.add_username_password("abc", "123", url_div)
        us, pw = dotnetrc.get_username_password(url_div)
        self.assertEqual(us, "abc")
    ...
    def test_061(self) -> None:
        ...
        self.assertEqual(pw, "uvw")
        #
        dotnetrc.set_username_password("abc", "pqr")
        dotnetrc.add_username_password("abc", "345", url_div)
        us, pw = dotnetrc.get_username_password(url_div)
        self.assertEqual(us, "abc")
    ...
    def test_431(self) -> None:
        ...
        with open(self.netrc(), "w") as f:
            ...
            f.write(f"machine *.intra password ijk\n")
        us, pw = dotnetrc.get_username_password(dev_div)
        self.assertEqual(us, "foo")
        self.assertEqual(pw, "pqr")
        us, pw = dotnetrc.get_username_password(dev_k8s)
        self.assertEqual(us, "foo")
        self.assertEqual(pw, "pqr")  # not "ijk"
    ...
    def test_901(self) -> None:
        cmd = ["./dotnetrc.py", "help"]
        out = subprocess.check_output(cmd)
        logg.info("OUT:\n%s", out)
        self.assertIn(b"x", out)
    def test_902(self) -> None:
        filename = dotnetrc.NETRC_FILENAMES[0]
        cmd = ["./dotnetrc.py", "help"]
        out = subprocess.check_output(cmd)
        logg.info("OUT:\n%s", out)
        self.assertIn(b"x", out)
    def test_903(self) -> None:
        filename = dotnetrc.NETRC_FILENAMES[0]
        cmd = ["./dotnetrc.py", "help", "--netcredentials", filename]
        out = subprocess.check_output(cmd)
        logg.info("OUT:\n%s", out)
        self.assertIn(b"x", out)
    def test_910(self) -> None:
        filename = dotnetrc.NETRC_FILENAMES[0]
        cmd = ["./dotnetrc.py", "--netcredentials", filename, "get", "extra.example"]
        out = subprocess.check_output(cmd)
        logg.info("OUT:\n%s", out)
    ...
    def test_911(self) -> None:
        filename = dotnetrc.NETRC_FILENAMES[0]
        cmd = ["./dotnetrc.py", "--netcredentials", filename, "store", "extra.example", "someuser", "somepass"]
        out = subprocess.check_output(cmd)
        logg.info("OUT:\n%s", out)
    ...
    def test_912(self) -> None:
        ...
        self.assertIn(b"username=someuser", out)
        self.assertIn(b"password=somepass", out)
        cmd = ["./dotnetrc.py", "--netcredentials", filename, "erase", "extra.example"]
        out = subprocess.check_output(cmd)
        logg.info("OUT:\n%s", out)
...
if __name__ == "__main__":
    ...
    # dotnetrc.netrc_logg.setLevel(("-v" not in sys.argv and logging.INFO or logging.DEBUG))
    # unittest.main()
    ...
    dotnetrc.netrc_logg.setLevel(max(0, logging.INFO - 10 * opt.verbose + 10 * opt.quiet))
    if not args:
        args = ["test_*"]
